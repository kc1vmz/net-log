...
@Singleton
public class OperatorAffiliationReportAccessor {
    private final static int MAX_ROWS = 28;
    private static final Logger logger = LogManager.getLogger(OperatorAffiliationReportAccessor.class);


    public String generateReport(Section section, List<SectionOperator> members, OperatorAffiliationReportType reportType, String title) {
        try {
            return createReport(section, members, reportType, title);
        } catch (Exception e) {
    ...
    public String createReport(Section section, List<SectionOperator> members, OperatorAffiliationReportType reportType, String title) throws FileNotFoundException {
        String filename = getUniqueFileName(getTempReportDir(), "pdf");
        try {
            int pageTotal = 1;

            // remove participants from all members
            members = removeMembersByReportType(members, reportType);
            int memberCount = members.size();

            pageTotal = (((memberCount / 2) + 1) / MAX_ROWS) + 1;

            PdfDocument pdf = new PdfDocument(new PdfWriter(new FileOutputStream(filename)));
            Document document = new Document(pdf);
            document.setMargins(30, 32, 30, 32);

            String headerCount = String.format("IDENTIFIED MEMBERS (%d TOTAL)", memberCount);
            int page = 1;
            for (; page <= pageTotal; page++) {
                document.add(addDocumentHeader(title));
                document.add(addInfoHeader(section.getName()));
                document.add(addMonthlyReportBanner(headerCount));

                Table eventListHeader = addMemberHeader();
                addMemberRows(eventListHeader, members, page);
    ...
    private List<SectionOperator> removeMembersByReportType(List<SectionOperator> members, OperatorAffiliationReportType reportType) {
        ...
        for (SectionOperator member : members) {
            ...
            switch (reportType) {
                ...
                case MEMBERS:
                    ...
                    break;
                case LEADERSHIP:
                    if (member.getMembershipType().equals(MembershipType.LEADERSHIP)) {
                        bAdd = true;
                    }
                    break;
                case NTS:
     ...
     *
     */
    private void addMemberRows(Table table, List<SectionOperator> members, int page) {
        table.useAllAvailableWidth();
