...
public class Report309Entry {
    ...
    public Report309Entry(Participant participant) {
        from = participant.getOperator().getCallsign();
        startTime = DateStrUtils.getTimeStr(participant.getStartTime());
        endTime = DateStrUtils.getTimeStr(participant.getEndTime());
        message = String.format("%s:%s:%sW", participant.getPrimaryPower(), participant.getBackupPower(), participant.getTransmitPower());
    }
